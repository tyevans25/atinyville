...
function secondsUntilEndOfNextKSTDay(): number {
  ...
  const endOfNextDay = new Date(Date.UTC(
    ...
    kstNow.getUTCMonth(),
    kstNow.getUTCDate() + 2, // +2 = end of next KST day
    0, 0, 0
  ) - 9 * 60 * 60 * 1000) // convert back to UTC
...
export async function GET() {
  try {
    ...
    if (userId && missions.length > 0) {
      ...
      return NextResponse.json({ missions: missionsWithProgress, date: today })
    }
...
export async function POST(request: Request) {
  try {
    ...
    await kv.set(missionsKey, missions, { ex: ttl })
    ...
    return NextResponse.json({ success: true, missions, date: today })
  } catch (error) {
    console.error('Error setting missions:', error)
    return NextResponse.json({ error: 'Failed to set missions' }, { status: 500 })
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!missionId || streams === undefined) {
      return NextResponse.json({ error: 'Mission ID and streams required' }, { status: 400 })
    }
    ...
    // Progress also expires at end of next KST day
    await kv.set(progressKey, streams, { ex: ttl })

    return NextResponse.json({ success: true, missionId, streams, date: today })
  } catch (error) {

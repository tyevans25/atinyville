...
export async function POST(request: Request) {
  try {
    ...
    // Check if this is a manual refresh for specific user
    let body: any = {}
    try {
      body = await request.json()
    } catch {
      // No body means regular cron
    }

    const { userId: manualUserId, manual, tier: manualTier } = body

    if (manual && manualUserId) {
      console.log(`🔄 Manual refresh for user ${manualUserId}`)

      // Process single user immediately
    ...
    console.log(`📊 Checking ${userKeys.length} users for tier-based processing...`)

    // Filter users based on tier intervals
    for (const key of userKeys) {
      ...
      if (timeSinceLastCheck >= interval * 60 * 1000) {
        ...
        console.log(`⏭️  Skipping ${userId} (${tier} tier) - ${minutesRemaining}min until next check`)
      }
    }

    console.log(`✅ Processing ${usersToProcess.length} users based on tier intervals`)

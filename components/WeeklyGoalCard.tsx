...
const pickGif = (cur: number, tar: number) => shuffle(cur/tar >= 0.5 ? celebrationGifs : motivationGifs)[0]
const gifMsg  = (cur: number, tar: number) => { const p=cur/tar; return p>=1?"🎉 Week Complete!":p>=0.5?"💪 Halfway there!":"🏴‍☠️ Let's go ATINY!" }
...
export default function WeeklyGoalCard() {
  ...
  useEffect(() => {
    fetchGoalData(); calcDays()
    const i1 = setInterval(fetchGoalData, 10*60*1000)
    const i2 = setInterval(calcDays, 60*1000)
    return () => { clearInterval(i1); clearInterval(i2) }
  }, [isSignedIn])

  useEffect(() => {
    if (goalData) { setCurrentGif(pickGif(safe(goalData.current), safe(goalData.target))); setGifError(false) }
  }, [goalData])

  const calcDays = () => {
    const now = new Date(); const kst = new Date(now.getTime()+9*3600000)
    const dow = kst.getUTCDay(); const d = (4-dow+7)%7
    setDaysRemaining(d === 0 ? 7 : d)
  }

  const fetchGoalData = async () => {
    try { const r = await fetch("/api/community-weekly-goal"); setGoalData(r.ok ? await r.json() : null) }
  ...
  const header = (
    <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.02)" }}>
      <Calendar style={{ width: 15, height: 15, color: "#58a6ff" }} />
      <span style={{ color: "#e6edf3", fontWeight: 700, fontSize: 14 }}>Weekly Community Goal</span>
    </div>
  )

  if (loading) return <div>{header}<div style={{padding:20}}><p style={{color:"#484f58",fontSize:13}}>Loading...</p></div></div>
  if (!goalData) return <div>{header}<div style={{padding:"32px 20px",textAlign:"center"}}><Target style={{width:32,height:32,color:"rgba(255,255,255,0.1)",margin:"0 auto 10px"}}/><p style={{color:"#8b949e",fontSize:13}}>No weekly goal set</p></div></div>
    ...
    <div>
      ...
      <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "5px 10px", alignSelf: "flex-start" }}>
          <Calendar style={{ width: 12, height: 12, color: "#58a6ff" }} />
          <span style={{ color: "white", fontSize: 12, fontWeight: 600 }}>{daysRemaining} {daysRemaining === 1 ? "day" : "days"} remaining</span>
        </div>

        <div>
          <p style={{ color: "#484f58", fontSize: 11, margin: "0 0 2px" }}>This Week's Goal</p>
          <p style={{ color: "white", fontWeight: 800, fontSize: 17, margin: 0 }}>Stream ANY ATEEZ songs</p>
          <p style={{ color: "#484f58", fontSize: 11, margin: "2px 0 0" }}>Resets Thursday 12AM KST</p>
        </div>
